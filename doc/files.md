...
macOS    | `$HOME/Library/Application Support/LebowskisCoin/`
...
`./`               | `lebowskiscoin.conf`       | User-defined configuration settings for `lebowskiscoind` or `lebowskiscoin-qt`; can be specified by `-conf` option
`./`               | `lebowskiscoind.pid`        | Stores the process ID (PID) of `lebowskiscoind` or `lebowskiscoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `lebowskiscoind` or `lebowskiscoin-qt`
...
`lebowskiscoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
