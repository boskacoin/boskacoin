...
**Optional dependencies** :
```bash
# Qt (required for lebowskiscoin-qt GUI)
...
According to installed dependencies, the following steps will compile `lebowskiscoind`, `lebowskiscoin-cli` and `lebowskiscoin-qt`.
...
#### Enable Qt GUI
Create `lebowskiscoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
