...
This tutorial will help you to go through the basics to use Dogecoin Core after you completed the [installation instructions](/INSTALL.md). You now have `lebowskiscoind` or `lebowskiscoin-qt` executables available to run a node, and `lebowskiscoin-cli`/`lebowskiscoin-tx` tools to help you transact DOGE.
...
> ```console
> shibetoshi:~$ /path/to/lebowskiscoin-cli [arguments ...]
> ```
...
To start your node, you can run an headless server using `lebowskiscoind`:
```console
shibetoshi:~$ lebowskiscoind -daemon
```

Or you can use the Graphical User Interface (GUI), `lebowskiscoin-qt`:
```console
shibetoshi:~$ lebowskiscoin-qt
```
...
> **Note:** The rest of this guide assumes the use of an headless node. The RPC server is not exposed with `lebowskiscoin-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `lebowskiscoin-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.
...
The Dogecoin Core installation provides the `lebowskiscoin-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `22555`, so that other tools and libraries can interact with it.

To have an overview of the available commands, use the `help` command:

```console
#List all commands
shibetoshi:~$ lebowskiscoin-cli help

#Get help for a specific command
shibetoshi:~$ lebowskiscoin-cli help COMMAND
```
...
```console
shibetoshi:~$ lebowskiscoin-cli getaddressesbyaccount ""
[
...
```console
shibetoshi:~$ lebowskiscoin-cli getnewaddress
...
```console
shibetoshi:~$ lebowskiscoin-cli backupwallet /path/of/wallet/backup
```
...
```console
#Syntax
shibetoshi:~$ lebowskiscoin-cli getbalance "*" minconf
```
...
```console
shibetoshi:~$ lebowskiscoin-cli getbalance "*" 5
421.552000
```
...
```console
#Syntax
shibetoshi:~$ lebowskiscoin-cli sendtoaddress address amount

#Example
shibetoshi:~$ lebowskiscoin-cli sendtoaddress nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh 420
```
...
```console
#Syntax
shibetoshi:~$ lebowskiscoin-cli listunspent minconf maxconf '["address", ...]'

#Example
shibetoshi:~$ lebowskiscoin-cli listunspent 1 9999999 '["nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P"]'
[
  {
...
shibetoshi:~$ lebowskiscoin-cli createrawtransaction "$utxos_to_use" '{"address":amount, ...}'
...
shibetoshi:~$ lebowskiscoin-cli createrawtransaction "$utxos_to_use" '{"nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh":69, "nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P": 30.999}'
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `lebowskiscoin-cli decoderawtransaction` or `lebowskiscoin-tx -json` to convert the content to JSON format.
...
```console
#Syntax
shibetoshi:~$ lebowskiscoin-cli signrawtransaction encoded_transaction

#Example
shibetoshi:~$ lebowskiscoin-cli signrawtransaction "01000000013324faf8f03695261611669f5bdd93b68a86cf02db12dc46642ed50666ed69b80000000000ffffffff0200a5459b010000001976a91418a89ee36293f15c4db4c01173babd579243161188ac60b8c4b8000000001976a914c6977da37560e1432c2e14e16952981a4c272cac88ac00000000"
{
...
```console
#Syntax
shibetoshi:~$ lebowskiscoin-cli sendrawtransaction signed_transaction

#Example
shibetoshi:~$ lebowskiscoin-cli sendrawtransaction 
...
shibetoshi:~$ lebowskiscoin-cli getblockhash 69
3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b

#Get block data
shibetoshi:~$ lebowskiscoin-cli getblock 3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b
{
...
```console
#Syntax
shibetoshi:~$ lebowskiscoin-cli getrawtransaction txid verbose

#Example
shibetoshi:~$ lebowskiscoin-cli getrawtransaction 695ce4208fa7a87ef9e99805b0910dc129058ecdceb5cef7e25f71dcdc7936db 1
{
...
Using `lebowskiscoind -help` will display all available configuration parameters that can be added as arguments:

**Command example :**
```console
shibetoshi:~$ lebowskiscoind -daemon -paytxfee=0.01 -sendfreetransactions=1 -maxconnections=150
```
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `lebowskiscoind -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `lebowskiscoin-cli`.
...
You may need to specify `-datadir` also when using `lebowskiscoin-cli`.
